...
// 8. УМНЫЙ КОНТРОЛЬ МУЗЫКИ: Пауза при уходе и Возобновление при возврате
document.addEventListener('visibilitychange', () => {
    if (music) {
        if (document.hidden) {
            // Если пользователь свернул браузер или переключил вкладку — пауза
            music.pause();
        } else {
            // Если пользователь вернулся на вкладку — продолжаем играть
            // .play() может не сработать без клика, но так как пользователь 
            // уже взаимодействовал с сайтом ранее, браузер должен разрешить возобновление.
            music.play().catch(e => console.log("Музыка ждет клика для возобновления"));
        }
    }
});

// На мобильных устройствах событие 'pagehide' срабатывает при переходе по ссылке
