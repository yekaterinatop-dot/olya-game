...
// --- ОТКЛЮЧЕНИЕ МУЗЫКИ ПРИ ПЕРЕХОДЕ ---
    // Ищем и по ID, и по классу на всякий случай
    const siteLink = document.getElementById('top-banner-link') || document.querySelector('.top-banner');

    if (siteLink) {
        siteLink.addEventListener('click', () => {
            console.log("Клик по баннеру, пытаюсь остановить музыку...");
            
            // 1. Пытаемся остановить через переменную music
            if (typeof music !== 'undefined' && music) {
                music.pause();
                music.currentTime = 0; // Сбрасываем в начало
            } 
            
            // 2. На всякий случай ищем вообще все теги audio на странице и стопаем их
